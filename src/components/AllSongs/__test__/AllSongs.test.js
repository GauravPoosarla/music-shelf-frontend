...
describe('AllSongs', () => {
  ...
  it('should navigate to /genre when the button is clicked', () => {
    ...
    const button = screen.getByAltText('icon-grid');
    fireEvent.click(button);
    expect(mockNavigate).toHaveBeenCalledWith('/genre');
  });

  it('should change the genreClicked state when the button is clicked', () => {
    makeRequest.mockResolvedValue(mockSongs);
    render(
      <SongDataProvider value={{ songs: mockSongs, setSongs: jest.fn() }}>
        <AllSongs />
    ...
    const button = screen.getByAltText('icon-grid');
    fireEvent.click(button);
    // TODO: check if the state variable is changed after clicking the button
  });
